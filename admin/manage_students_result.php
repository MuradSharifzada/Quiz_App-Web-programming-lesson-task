<?php
// admin/manage_students_result.php
...
// --- PART 1: Fetch summary of all quizzes ---
$quiz_summaries = [];
try {
    $sql_summary = "
        SELECT 
    ...
    // Validate selected quiz_id and get title if quiz_id selected
    if ($selected_quiz_id) {
        $found = false;
        ...
        if (!$found) {
             $error = "The selected Quiz ID is invalid.";
...
// --- PART 2: Fetch detailed results for selected quiz ---
$detailed_results = [];
if ($selected_quiz_id) {
    try {
        $sql_details = "
        ...
        $stmt_details->execute([$selected_quiz_id]);
...
<body>
    <div class="header">
        <h1>Student Results Dashboard (Admin)</h1>
        <div>
        ...
        <?php if (!$selected_quiz_id): ?>
            <div class="results-section">
                <?php if (empty($quiz_summaries)): ?>
                    <p>No quizzes found or no results available.</p>
                <?php else: ?>
